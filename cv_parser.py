...
def convert_docx_to_html(input_path: str) -> str:
    ...
    try:
        ...
        # If pandoc fails, clean up and raise the error
        os.unlink(temp_html)
        raise RuntimeError(f"Pandoc conversion failed: {str(e)}")
    except Exception as e:
        # For any other error, clean up and raise
        os.unlink(temp_html)
        raise
            ...
            for row in rows:
                ...
                if len(cells) == 2:
                    ...
                    if current_section in ["Erfarenhet", "Experience"]:
                        ...
                        # Process paragraphs for description and tech stack
                        paragraphs = right_cell.find_all('p')
                        
                        # Separate description paragraphs from tech stack items
                        ...
                        for para in paragraphs:
                            ...
                            if para_text:
                                para_text = ' '.join(para_text.split())
                                # If paragraph is short (less than 50 chars), it's likely a tech stack item
                                if len(para_text) < 50:
                                    tech_stack_items.append(para_text)
                                else:
                                    description_paragraphs.append(para_text)
                        
                        # Add description paragraphs to markdown
                        for para in description_paragraphs:
                            markdown.append(para)
                            markdown.append("")  # Blank line after paragraph
                        
                        # Add tech stack items to markdown with better formatting
                        if tech_stack_items:
                            markdown.append("**Tech Stack:**")
                            # Format tech stack items as a comma-separated list
                            tech_stack_text = ", ".join(tech_stack_items)
                            markdown.append(tech_stack_text)
                            markdown.append("")  # Blank line after tech stack
